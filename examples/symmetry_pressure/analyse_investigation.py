...
for row, task in enumerate(tasks_present):
    ...
    for col, cond in enumerate(task_conds_list):
        ...
        gens_s = sorted(all_syms_by_gen.keys())
        sym_data = [all_syms_by_gen[g] for g in gens_s if all_syms_by_gen[g]]
        gen_lbls = [str(g) for g in gens_s if all_syms_by_gen[g]]
        if sym_data:
            positions = list(range(1, len(sym_data) + 1))
            parts = ax.violinplot(sym_data, positions=positions, showmedians=True)
            for pc in parts.get("bodies", []):
                pc.set_facecolor(color)
                pc.set_alpha(0.5)
            ax.set_xticks(positions)
            ax.set_xticklabels(gen_lbls, rotation=45 if len(gen_lbls) > 10 else 0, fontsize=6)
