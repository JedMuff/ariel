"""
Body + brain co-evolution on the randomised-waypoints task.
...
Each outer generation:
  1. Sample a fresh waypoint layout (deterministic per-generation seed).
  2. For each body candidate (parents + offspring): run an inner CMA-ES brain
     search across worker processes; record best brain fitness.
  3. (mu+lambda) survivor selection: keep best mu of (mu+lambda).

Waypoints are mocap bodies (no contact dynamics) — the robot passes through them.
Vision input is the onboard body camera (the overview camera is video-only).

Fitness: lower is better; same convention as 5_randomized_waypoints.py.
  Incomplete: min_dist_to_current - waypoints_reached * 10
  Complete:   -waypoints_reached * 10 - (DURATION - completion_time) / DURATION
"""
...
def _train_body_serial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"fitness": best_fit, "weights": best_w.tolist() if best_w is not None else None}
...
def learn_brain(
    ...
    """Run CMA-ES on the brain for one body. Returns (best_fitness, best_weights)."""
...
class BodyBrainEvolution:
    ...
    def reproduction(self, population: Population) -> Population:
        ...
        while len(offspring) < LAM:
            ...
            child.tags = {"ps": False, "valid": True, "best_brain": []}
    ...
    def evaluate(self, population: Population) -> Population:
        ...
        self.gen_waypoints = sample_waypoints(gen_rng, n=NUM_WAYPOINTS, radius=ARENA_RADIUS)
        ...
        if STRATEGY == "across":
            ...
            for idx, ind in enumerate(to_eval):
                t0 = time.time()
                best_fit, best_w = learn_brain(
                    ind.genotype["morph"], self.gen_waypoints, self.executor,
                )
                ...
                console.log(
                    ...
                    f"params={len(best_w) if best_w is not None else 0}  ({elapsed:.1f}s)"
                )
