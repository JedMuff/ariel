"""
Train the amputee gecko (front-right arm removed) from random initial weights,
then record side-view and top-view GIF + MP4 of the best result.

Companion to gecko_amputee_retrain.py — identical setup except the CMA-ES
starts from a random weight vector instead of the pruned gecko_forward weights.

Weights are saved to __data__/gecko_amputee_random_retrain/best_weights.npy.

Usage
-----
    uv run python examples/re_book/gecko_amputee_random_retrain.py
    ...
    uv run python examples/re_book/gecko_amputee_random_retrain.py --no-video
"""
...
def _get_ctx() -> dict[str, Any]:
    ...
    if _ctx is None:
        ...
        net         = Network(input_size=state_dim, output_size=model.nu, hidden=_WORKER_HIDDEN)
...
def _evaluate(weights: np.ndarray) -> float:
    ...
    return run_episode(ctx["model"], ctx["data"], ctx["net"], _WORKER_DURATION)
...
def evolve(args) -> np.ndarray:
    ...
    init_weights = np.random.uniform(-0.5, 0.5, size=num_params)

    pop = max(args.pop, 4 + int(3 * np.log(max(num_params, 2))))
    if pop % 2 != 0:
        pop += 1

    console.rule("[bold cyan]Gecko Amputee Random Retrain — CMA-ES[/bold cyan]")
    console.log(f"params={num_params}  pop={pop}  budget={args.budget} gens  "
                f"workers={args.workers}  dur={args.dur}s")

    param     = ng.p.Array(init=init_weights).set_mutation(sigma=0.3)
    ...
    out_dir = Path(args.out_dir) / "gecko_amputee_random_retrain"
...
def record_video(best_weights: np.ndarray, args) -> None:
    ...
    if not args.no_gif:
        _save_gif(side_frames, out / "gecko_amputee_random_retrain_side.gif", args.fps)
        _save_gif(top_frames,  out / "gecko_amputee_random_retrain_top.gif",  args.fps)

    if not args.no_mp4:
        _save_mp4(side_frames, out / "gecko_amputee_random_retrain_side.mp4", args.fps)
        _save_mp4(top_frames,  out / "gecko_amputee_random_retrain_top.mp4",  args.fps)
...
def main():
    ...
    parser.add_argument("--dur",            type=float, default=8.0,  help="Episode duration (s) for training")
    parser.add_argument("--hidden",         type=int,   default=32)
    parser.add_argument("--seed",           type=int,   default=42)
    parser.add_argument("--out-dir",        default="__data__")
    parser.add_argument("--no-train",       action="store_true", help="Skip training; load weights from out-dir/gecko_amputee_random_retrain/best_weights.npy")
    ...
    if args.no_train:
        ...
        net         = Network(input_size=state_dim, output_size=model.nu, hidden=args.hidden)
        num_params  = sum(p.numel() for p in net.parameters())
        best_weights = np.random.uniform(-0.5, 0.5, size=num_params)
        console.log("Skipping training — using random weights")
    else:
