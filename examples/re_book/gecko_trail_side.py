"""
Ghost-trail side-view image of the gecko's gait.
...
-----
    ...
    uv run python examples/re_book/gecko_trail_side.py \\
...
def _build_gecko_world():
    ...
    phase_dim   = 2
    state_dim   = len(robot_state) + phase_dim
...
def _make_side_camera(centre_x: float, centre_y: float,
                      distance: float = 1.8) -> mujoco.MjvCamera:
    cam = mujoco.MjvCamera()
    cam.type      = mujoco.mjtCamera.mjCAMERA_FREE
    cam.lookat    = np.array([centre_x, centre_y, 0.12], dtype=np.float64)
...
def _composite(canvas: np.ndarray, frame: np.ndarray, alpha: float,
               render_bg: np.ndarray) -> np.ndarray:
    # Mask out background pixels so earlier ghosts aren't overwritten by the
    # frame's own sky/floor. Only blend pixels that belong to the gecko.
    diff = np.abs(frame.astype(np.float32) - render_bg.astype(np.float32)).max(axis=2)
    mask = (diff > 12).astype(np.float32)[:, :, np.newaxis]

    effective_alpha = mask * alpha
    out = effective_alpha * frame.astype(np.float32) + (1.0 - effective_alpha) * canvas.astype(np.float32)
...
def render_trail(args) -> None:
    ...
    # Each spawned robot occupies a contiguous block of qpos starting after
    # the free joint (7 dofs) + the hinge joints from the previous robots.
    mujoco.mj_resetData(model, data)
    ...
    centre_x     = float((min(s[0] for s in snapshots) + max(s[0] for s in snapshots)) / 2)
    centre_y     = float(np.median([s[1] for s in snapshots]))
    x_span       = max(s[0] for s in snapshots) - min(s[0] for s in snapshots)
    cam_distance = max(1.5, x_span * 0.75)
    ...
    cam = _make_side_camera(centre_x, centre_y, distance=cam_distance)
...
def main() -> None:
    ...
    parser.add_argument("--output",       default="__data__/gecko_trail_side.png")
