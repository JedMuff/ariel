"""
Train the full gecko from random initial weights, then record side-view and
top-view GIF + MP4 of the best result.

This is a baseline companion to gecko_amputee_retrain.py — same CMA-ES setup,
same network architecture, but the full gecko morphology and random init.

Weights are saved to __data__/gecko_random_retrain/best_weights.npy.

Usage
-----
    uv run python examples/re_book/gecko_random_retrain.py
    ...
    uv run python examples/re_book/gecko_random_retrain.py --no-video
"""
...
def evolve(args) -> np.ndarray:
    ...
    console.rule("[bold cyan]Gecko Random Retrain — CMA-ES[/bold cyan]")
    ...
    out_dir = Path(args.out_dir) / "gecko_random_retrain"
...
def record_video(best_weights: np.ndarray, args) -> None:
    ...
    if not args.no_gif:
        _save_gif(side_frames, out / "gecko_random_retrain_side.gif", args.fps)
        _save_gif(top_frames,  out / "gecko_random_retrain_top.gif",  args.fps)

    if not args.no_mp4:
        _save_mp4(side_frames, out / "gecko_random_retrain_side.mp4", args.fps)
        _save_mp4(top_frames,  out / "gecko_random_retrain_top.mp4",  args.fps)
...
def main():
    ...
    parser.add_argument("--no-train",       action="store_true", help="Skip training; load weights from out-dir/gecko_random_retrain/best_weights.npy")
